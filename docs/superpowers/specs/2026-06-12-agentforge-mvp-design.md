...
5. 管理员在平台内维护模板 skills，仅支持新增完整 skill 和删除 skill，不支持编辑已有 skill。
...
模板 skill 仅支持新增和删除。`POST /api/admin/templates/{id}/skills` 创建一个完整 skill，`GET` 查看 skill，`DELETE` 删除整个 skill。MVP 不支持文件级编辑，也不支持整体替换已有 skill；需要修改时，管理员删除旧 skill 后重新新增。
