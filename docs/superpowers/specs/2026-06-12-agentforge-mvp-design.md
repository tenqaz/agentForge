...
这里的 Worker 指 Go 后端里的后台任务执行器，不是一个前端概念，也不是独立产品模块。第一版可以把 API server 和 worker 放在同一个 Go 进程中运行，通过 goroutine 消费任务表；部署成熟后可以拆成独立 worker 进程。API 请求只负责创建记录、校验权限、写入任务并返回当前资源状态；容器创建、Hermes home 生成、gateway 启动、微信二维码轮询等耗时操作由 worker 异步推进。
...
flowchart LR
    ...
    API --> Jobs[任务表 runtime_jobs / channel_jobs]
...
任务表是 API 与 worker 的边界。API 不直接等待任务完成；前端通过资源状态接口轮询或通过 SSE 接收状态变化。
...
- 对于微信扫码，Worker 参考 Hermes `gateway/platforms/weixin.py` 的 `qr_login()` 流程，通过 iLink `get_bot_qrcode` 和 `get_qrcode_status` 获取二维码与扫码状态，并同步到 `channel_pairing_sessions`。
...
13. Worker 更新该 Agent 的 `config.yaml` 和 `.env`，写入微信渠道配置。
...
sequenceDiagram
    ...
    W->>IX: get_bot_qrcode(bot_type=3)
...
- `PUT /api/agents/{id}/runtime`
- `DELETE /api/agents/{id}/runtime`
...
- 获取 Agent 渠道锁。
- 创建或复用未过期的配对会话。
- 调用 iLink `get_bot_qrcode` 获取二维码。
- 将渠道状态更新为 `qr_pending`。
- 轮询 iLink `get_qrcode_status`。
...
- `PUT /runtime` 和 `DELETE /runtime` 不能同时执行。
- `provision_agent` 和 `restart_runtime` 不能同时执行。
