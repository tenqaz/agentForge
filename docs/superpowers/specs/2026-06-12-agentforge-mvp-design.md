...
- 生成 Weixin 配置时，默认私信策略为 `allowlist`，默认群组策略为 `disabled`。平台只允许绑定该 Agent 的平台用户访问对应微信 Agent；群聊默认关闭。
...
- Go 后端生成 `$HERMES_HOME/config.yaml` 时写入 `gateway.platforms.weixin.extra.account_id/base_url/dm_policy/group_policy/allow_from/group_allow_from` 等配置。
- 默认 `dm_policy=allowlist`，`group_policy=disabled`。`allow_from` 初始写入当前平台用户绑定的 Weixin user id；在无法提前知道 Weixin sender id 的阶段，连接完成后先只允许扫码得到的 `ilink_user_id`，后续通过平台授权流程追加。
...
sequenceDiagram
    ...
    W->>H: 更新 config.yaml / .env<br/>dm_policy=allowlist, group_policy=disabled
    W->>G: 启动或重启 gateway
    G->>IX: getupdates 长轮询
...
- 写入 Weixin 默认安全策略：`dm_policy=allowlist`、`group_policy=disabled`。
