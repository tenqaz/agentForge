...
func (w *ChannelWorker) ProcessJob(ctx context.Context, jobID string) error {
	...
	if agent.Status != "running" {
		...
		return errors.New("agent_not_running")
	}
	...
	if err != nil {
		return w.fail(ctx, job.ID, channel.ID, "qr_request_failed", err.Error(), channels.StatusError)
	}
	imagePath, err := w.writeQRImage(agent.HermesHomePath, session.ID, qr.QRCodeImageContent)
	if err != nil {
		return w.fail(ctx, job.ID, channel.ID, "qr_image_write_failed", err.Error(), channels.StatusError)
	}
	...
	for {
		...
		if err != nil {
			return w.fail(ctx, job.ID, channel.ID, "qr_status_failed", err.Error(), channels.StatusError)
		}
		...
		case weixin.StatusConfirmed:
			if err := w.writeConfirmedCredentials(agent.HermesHomePath, status); err != nil {
				return w.fail(ctx, job.ID, channel.ID, "credential_write_failed", err.Error(), channels.StatusError)
			}
			if err := w.runner.Stop(ctx, agent.RuntimeID); err != nil {
				return w.fail(ctx, job.ID, channel.ID, "runtime_restart_failed", err.Error(), channels.StatusError)
			}
...
func (w *ChannelWorker) loadJobContext(ctx context.Context, jobID string) (channelWorkerJob, channels.Channel, channelWorkerAgent, error) {
	...
	if err != nil {
		return channelWorkerJob{}, channels.Channel{}, channelWorkerAgent{}, err
	}
...
func (w *ChannelWorker) ensureSession(ctx context.Context, channelID string) (channels.PairingSession, error) {
	...
	return w.channels.CreatePairingSession(ctx, channelID, time.Now().Add(5*time.Minute).UTC().Format(time.RFC3339))
}
...
func (w *ChannelWorker) writeConfirmedCredentials(homePath string, status weixin.QRStatusResponse) error {
	...
	return os.WriteFile(accountFile, payload, 0o644)
}
